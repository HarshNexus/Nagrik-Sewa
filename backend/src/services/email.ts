...
const verifyResendConfig = () => {
  if (!process.env.RESEND_API_KEY) {
    console.warn('[EMAIL] Resend API key not configured - email sending will use fallback mode');
    return false;
  }
...
/**
 * Sends an email asynchronously using Resend
 * This function returns immediately with proper error handling
 */
export const sendEmail = async (options: EmailOptions): Promise<{ success: boolean; messageId?: string; error?: string }> => {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      try {
        const response = await resend.emails.send({
          from: process.env.EMAIL_FROM || 'Nagrik Sewa <noreply@resend.dev>',
          to: options.to,
        ...
        if (response.error) {
          console.error(`❌ [EMAIL-SEND] Resend failed:`, response.error);
          return { 
            success: false, 
        ...
        console.log(`[EMAIL-SEND]   Message ID: ${response.data?.id}`);
        return { success: true, messageId: response.data?.id };
      } catch (resendError) {
        const errorMsg = resendError instanceof Error ? resendError.message : String(resendError);
        console.error(`❌ [EMAIL-SEND] Resend error: ${errorMsg}`);
    ...
    console.log('[EMAIL-SEND] Resend API key not configured - using development mode');
    ...
    // Log OTP if this is an OTP email
    if (options.data?.otp && options.template?.includes('otp')) {
      console.log('');
